module.exports = {
  ...
  extendedDescription: `To ease testing and development, if the provided "to" email address ends in "@example.com", then the email message will be written to the terminal instead of actually being sent. (Thanks [@simonratner](https://github.com/simonratner)!)`,

  inputs: {
    template: {
      description:
        "The relative path to an EJS template within our `views/emails/` folder -- WITHOUT the file extension.",
      extendedDescription: `Use strings like "foo" or "foo/bar", but NEVER "foo/bar.ejs" or "/foo/bar".  For example, "internal/email-contact-form" would send an email using the "views/emails/internal/email-contact-form.ejs" template.`,
