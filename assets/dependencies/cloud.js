...
(function(factory, exposeUMD){
...
})((_, io, $, SAILS_LOCALS, location, File, FileList, FormData) => {
  ...
  function _representsOneOrMoreFiles(that) {
    ...
    return (
      ...
      (
        ...
        (_.isArray(that) && that.length > 0 && _.all(that, (item) => { return File? _.isObject(item) && item instanceof File : false; }))||
        (File? _.isObject(that) && _.isObject(that.file) && that.file instanceof File : false)||
        (_.isArray(that) && that.length > 0 && _.all(that, (item) => { return File? _.isObject(item) && _.isObject(item.file) && item.file instanceof File : false; }))
      )
    );
  }//ƒ
  ...
  Cloud.setup = function(options) {
    ...
    // Interpret methods
    var methods = _.reduce(options.methods, (memo, appLevelSdkEndpointDef, methodName) => {
      ...
      var _helpCallCloudMethod = function (argins) {
        ...
        var deferred = {
          ...
          toPromise: function (){
            ...
            _promise = new Promise(((resolve, reject) => {// eslint-disable-line no-undef
              try {
                deferred.exec((err, resultMaybe) => {
                  if (err){
          ...
          exec: function (exitCallbacks){
            ...
            (function _makeAjaxCallWithAppropriateProtocol(proceed){
              ...
              if (FormData && textParamsByFieldName) {
                ...
                _.each(textParamsByFieldName, (value, fieldName) => {
              ...
              switch (requestInfo.protocolName) {
                ...
                case "jQuery": return (function _doAjaxWithJQuery(){
                  ...
                  // > to get it to be losslessly encoded for use in backend code.
                    ...
                    _.each(uploadsByFieldName, (fileOrFileList, fieldName) => {
                  ...
                  // Otherwise, attach params as a JSON-encoded request body.
                    ...
                    // > including `NaN`, etc.
                    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                    var sanitizedTPBFN = _.mapValues(textParamsByFieldName, (value) => {
                      var sanitizedValue;
                      if (_.isArray(value)) {
                        sanitizedValue = _.clone(value);
                        _.remove(sanitizedValue, (item) => {
                ...
                case "io.socket": return (function _doAjaxWithSocket(){
                  ...
                  if (File && requestInfo.params) {
                    _.each(requestInfo.params, (value, fieldName) => {
                      if (_representsOneOrMoreFiles(value)) {
              ...
              var matchingLifecycleInstruction = _.find(requestInfo.lifecycleInstructions, (lifecycleInstruction) => {
              ...
              })((err, resultFromInterceptOrTolerate) => {
        ...
        var routeParameters = {};
        requestInfo.url.replace(/(\:[^\/\:\.\?]+\??)/g, ($all, $1) => {
        ...
        requestInfo.url = requestInfo.url.replace(/(\:[^\/\:\.\?]+\??)/g, ($all, $1) => {
          var routeParamName = $1.replace(/^\:/, "").replace(/\??$/, "");
      ...
      memo[methodName] = function () {
        ...
        var argins = _.reduce(arguments, (argins, argin, i) => {
